...
type MessageGroup struct {
	...
	GroupMembers   []User          `json:"users"         bson:"users"`
...
func CreateGroup(groupName string, adminUserID string) (string, error) {
	...
	// Add the admin user to the group
	usersCollection := globals.MongoClient.Database(database).Collection(CollectionName)
	...
	if err != nil {
		globals.Logger.Error().Err(err).Msg("Failed to add admin user to message group")
		return "", err
	}

	// Ensure result.InsertedID is a string
	globals.Logger.Info().Msgf("Inserted message group with ID %v", result.InsertedID)
